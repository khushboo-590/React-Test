import React from 'react';

const Home = () => (
    <div className="max-w-[1140px] mx-auto p-6 bg-green-300  mt-10 ">
        <h1 className="text-4xl font-extrabold mb-6 text-center text-indigo-700">Welcome to Our Website</h1>
        <p className="text-gray-700 leading-relaxed text-lg">
            This is the Home page. Here you can introduce your application or organization,
            highlight key features, and engage your visitors with a warm welcome message.
            Use this space to set the tone and share your vision.
        </p>
        <p className="mt-4 text-gray-600 italic text-center">
            &quot;Delivering quality and excellence every step of the way.&quot;
        </p>
    </div>
);

export default Home;
